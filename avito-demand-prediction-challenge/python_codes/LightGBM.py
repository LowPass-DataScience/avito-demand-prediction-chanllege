...
catFeatures = ['region', 'city', 'parent_category_name', 'param_1', 'param_2', 'param_3', 
               'user_type', 'image_top_1', 'activation_weekday', 'region_city']
...
if __name__ == "__main__":
    ...
    vers = 'v5'
    ss = time.time()
    dataTrain = pd.read_feather(f'{resultPath}/all_data_train_{vers}.feather')
    dataTest = pd.read_feather(f'{resultPath}/all_data_test_{vers}.feather')
    target = pd.read_hdf(f'{dataPath}/basicData.h5', key='trainTarget', mode='r')
    ...
    pred_sub = pd.read_csv(f'{dataPath}/sample_submission.csv')
    pred_sub.deal_probability = predResult
    pred_sub.to_csv(f"{dataPath}/gbmResults/predResult_{vers}_rsidx{rs_idx}_rmse{rmse:.5f}.csv")
